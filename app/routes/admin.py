...
def upload_cover_image():
    ...
    try:
        ...
        # 检测存储类型
        is_r2 = hasattr(storage, 's3_client')
        ...
        if is_r2:
            ...
            try:
                with Image.open(BytesIO(img_data)) as img:
                    ...
                    # 上传到 R2
                    if storage.upload_fileobj(output, object_name, content_type):
                        image_url = storage.get_url(object_name)
                    else:
                        return jsonify({'success': False, 'message': '上传到 R2 失败'}), 500
            except Exception as e:
